...
# for sel_var in sel_var_array:
    # pathh = str('/home/gemeinsam_tmp/VACILT/latent_heat_output/comparison/'+str(sel_var))
    # sel_month = 'Jan'
    ...
    # for sel_phase in sel_phase_array:
        ...
        # for sel_tide in sel_tide_array:
            ...
            # for sel_ave in sel_ave_array:
                ...
                # fig.savefig(f'{pathh}/{sel_tide}_{sel_ave}_comparison_{sel_var}_{sel_phase}_{sel_month}.png', bbox_inches='tight')
...
infile_el = f'ensemble_tides_tem_el_Jan.nc'
infile_la = f'ensemble_tides_tem_la_Jan.nc'
ds_wo_lh_el = xr.open_dataset(f'{root_path}muam_mstober/{infile_el}')
ds_wo_lh_la = xr.open_dataset(f'{root_path}muam_mstober/{infile_la}')
ds_w_lh_el = xr.open_dataset(f'{root_path}latent_heat_output/{infile_el}')
ds_w_lh_la = xr.open_dataset(f'{root_path}latent_heat_output/{infile_la}')

mean_wo_lh_el, std_wo_lh_el = process(ds_wo_lh_el[f'DT_amp']+ds_wo_lh_el[f'SDT_amp']+ds_wo_lh_el[f'TDT_amp']+ds_wo_lh_el[f'QDT_amp'])
mean_wo_lh_la, std_wo_lh_la = process(ds_wo_lh_la[f'DT_amp']+ds_wo_lh_la[f'SDT_amp']+ds_wo_lh_la[f'TDT_amp']+ds_wo_lh_la[f'QDT_amp'])
mean_w_lh_el, std_w_lh_el = process(ds_w_lh_el[f'DT_amp']+ds_w_lh_el[f'SDT_amp']+ds_w_lh_el[f'TDT_amp']+ds_w_lh_el[f'QDT_amp'])
mean_w_lh_la, std_w_lh_la = process(ds_w_lh_la[f'DT_amp']+ds_w_lh_la[f'SDT_amp']+ds_w_lh_la[f'TDT_amp']+ds_w_lh_la[f'QDT_amp'])
...
ax = axes[0]
p = mean_w_lh_el.plot(cmap = cmap, ax = ax, 
                             add_colorbar = False)#, levels = bounds)

std_w_lh_el.squeeze().plot.contour(levels = lvls, colors='g', ax = ax)
ax.text(0.6,0.1,'$\sigma_{max}$'+f'= {std_w_lh_el.max().values:2.3f}', fontsize=13, backgroundcolor=(1, 1, 1, 0.5), transform=ax.transAxes)


ax.set_title('With LH EL')
ax.xaxis.set_major_locator(xmajorLocator)
ax.yaxis.set_major_locator(ymajorLocator)
ax.grid(b=True, which='major', color='gray', linestyle='--')
...
std_w_lh_la.squeeze().plot.contour(levels = lvls, colors='g', ax = ax)
ax.text(0.6,0.1,'$\sigma_{max}$'+f'= {std_w_lh_la.max().values:2.3f}', fontsize=13, backgroundcolor=(1, 1, 1, 0.5), transform=ax.transAxes)

ax.set_ylabel('')
ax.set_title('With LH LA')
ax.grid(b=True, which='major', color='gray', linestyle='--')

plt.suptitle(f'Temp comparison for EL and LA with LH', y=1.02)
fig.savefig(f'EL_LA_with.png', bbox_inches='tight')
quit()
